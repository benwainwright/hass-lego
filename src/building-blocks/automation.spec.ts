...
import { RunQueue, BlockExecutionMode, EventBus, Executor, LegoClient } from "@core";
...
describe("automation.run", () => {
  it("throws an error if there is no event bus", async () => {
    ...
    await expect(automation.run(mockClient, input, undefined, triggerId)).rejects.toThrow()
  ...
  it("If result is undefined, throw an error", async () => {
    ...
    await expect(automation.run(mockClient, input, events, triggerId)).rejects.toThrow()
  ...
  it("If the executor aborts, return a response with continue set to false", async () => {
    ...
    mockExecutor.finished.mockRejectedValue(new ExecutionAbortedError("Execution was aborted"))
